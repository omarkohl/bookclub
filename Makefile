.PHONY: help build build-frontend build-backend lint lint-frontend lint-backend fmt fmt-check test test-backend test-frontend e2e e2e-headed dev-frontend dev-backend dev clean
...
dev: ## Start both backend and frontend dev servers
	...
	@bash -c 'trap "kill 0" EXIT; $(MAKE) dev-backend & $(MAKE) dev-frontend & wait -n; exit 1'

dev-frontend: ## Start Vite dev server (frontend only, proxies /api to :8080)
